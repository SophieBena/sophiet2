...
class Database:
    ...
    def __init__(self, rxn_data, modelDB=None,
                 ref_energy_phases=None,
                 phase_priors=None, rxn_priors=None,
                 use_rxn_studied=True,
                 infer_basic_rxns=False, ignore_kinetics=False,
                 contaminant_phases=None, rxn_trans_typ='logistic'):

        if modelDB is None:
            modelDB = model.Database()

        assert use_rxn_studied, (
            'current implimentation assumes use_rxn_studied'
        )

        assert not infer_basic_rxns, (
            'current implimentation assumes use_rxn_studied'
        )

        self._init_rxns(rxn_data, modelDB, use_rxn_studied)

        self._init_params(phase_priors, rxn_priors,
                          ref_energy_phases)
    ...
    def _init_rxns(self, rxn_data, modelDB, use_rxn_studied):
        ...
        if use_rxn_studied:
            ...
            # do stuff here to infer basic rxns
            # Where to put chem calls to generating rxn list
            pass

        self.use_rxn_studied = use_rxn_studied
    ...
    def _init_params(self, phase_priors, rxn_priors,
        ...
        self._set_phase_priors(phase_priors,
                               ref_energy_phases)
        self._set_rxn_priors(rxn_priors)

    def _set_phase_priors(self, phase_priors,
                          ref_energy_phases):
        ...
        assert np.all([ref_phs in phase_symbols
                 ...
                 'The ref_energy_phases provided are not valid.'
             )
