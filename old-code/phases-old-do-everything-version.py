...
__all__ = ['ThermoDBModel','ThermoDB','PhaseRxn','PhaseAssemblage','PurePhase']

#===================================================
class ThermoDBModel:
    def __init__(self, datasets, thermoDB=None, mask_phs_l=[],
        ...
        rxn_d_l, phasesym_l = self.get_rxn_phases(datasets)
    ...
    def init_param_scl(self):
        ...
        # param_scl_d['alpha_T_rxn_all']   =alpha_T_scl
        # param_scl_d['alpha_flux_rxn_all'] =alpha_H2O_scl

        # logGth_rxn = log(3/2*Rgas*1) + alpha_i*X_i
        rxn_l = self.rxn_l
        for ind,rxn in enumerate(rxn_l):
            # self._param_d['logGth0_rxn'+str(ind)] = -
            param_scl_d['activ_H_rxn'+str(ind)] = activ_H_scl
            param_scl_d['activ_V_rxn'+str(ind)] = activ_V_scl
            param_scl_d['alpha_0_rxn'+str(ind)]   = alpha_0_scl
            param_scl_d['alpha_flux_rxn'+str(ind)] = alpha_flux_scl
    ...
    def init_rxn_params(self,dTthresh0=1.0):
        ...
        # self._param_d['alpha_T_rxn_all']   = 0.0
        # self._param_d['alpha_flux_rxn_all'] = 0.0
        ...
        for ind,rxn in enumerate(rxn_l):
            ...
            self._param_d['activ_H_rxn'+str(ind)]   = 0.0
            self._param_d['activ_V_rxn'+str(ind)]   = 0.0
            self._param_d['alpha_0_rxn'+str(ind)]   = 0.0
            self._param_d['alpha_flux_rxn'+str(ind)] = 0.0
    ...
    @classmethod
    def get_rxn_phases(cls, datasets):
        rxns_studied = []
        for data in datasets:
            df = data['df']
            rxns_studied.extend(df['rxn_studied'].unique())

        rxns_studied = np.unique(rxns_studied)

        rxns_data = []
        phases = []
        for rxn in rxns_studied:
            rxn_data = ThermoDB.parse_rxn(rxn)
            rxns_data.append(rxn_data)
            phases.extend(rxn_data['prod_l'])
            phases.extend(rxn_data['reac_l'])

        phases = np.unique(phases)

        return rxns_data, phases
